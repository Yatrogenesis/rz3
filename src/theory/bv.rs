...
impl<'a> BitBlaster<'a> {
    pub fn new(
        ...
        Self { sat_solver, bv_vars, expr_to_bits, next_var }
    }

    fn new_var(&mut self) -> i32 {
        let v = *self.next_var; *self.next_var += 1;
    ...
    pub fn bit_blast(&mut self, expr: &Expr) -> Vec<i32> {
        ...
        let bits = match simplified {
            Expr::BvConst(val, width) => {
                ...
                for i in 0..width {
                    let v = self.new_var();
                    if (val >> i) & 1 == 1 { self.sat_solver.add_clause(vec![v]); }
                    else { self.sat_solver.add_clause(vec![-v]); }
                    b.push(v);
                }
                b
            }
            Expr::Var(name, Type::BitVec(width)) => {
                ...
                for i in 0..width {
                    let v = *self.bv_vars.entry((name.clone(), i)).or_insert_with(|| {
                        let nv = *self.next_var; *self.next_var += 1; nv
                    });
                    b.push(v);
                }
                b
            ...
            Expr::BvAdd(a, b) => {
                ...
                for (la, lb) in bits_a.into_iter().zip(bits_b) {
                    let sum = self.new_var(); let next_carry = self.new_var();
                    self.add_xor3(sum, la, lb, carry);
                    self.add_maj3(next_carry, la, lb, carry);
                    res.push(sum); carry = next_carry;
                }
                res
            }
            _ => vec![],
    ...
    fn word_level_simplify(&self, expr: &Expr) -> Expr {
        match expr {
            Expr::BvAnd(a, b) => {
                if let Expr::BvConst(0, w) = &**a { return Expr::BvConst(0, *w); }
                if let Expr::BvConst(0, w) = &**b { return Expr::BvConst(0, *w); }
            }
            Expr::BvAdd(a, b) => {
                if let Expr::BvConst(0, _) = &**a { return *b.clone(); }
                if let Expr::BvConst(0, _) = &**b { return *a.clone(); }
            }
            _ => {}
        }
        expr.clone()
