...
impl Default for MultivariatePolynomial {
    fn default() -> Self {
        Self::new()
    }
}
...
impl MultivariatePolynomial {
    ...
    pub fn mul(&self, other: &Self) -> Self {
        ...
        for (exps1, coeff1) in &self.terms {
            for (exps2, coeff2) in &other.terms {
                ...
                for (i, &exp) in exps2.iter().enumerate() {
                    new_exps[i] += exp;
                }
    ...
    pub fn resultant(&self, other: &Self, var_idx: usize) -> Self {
        ...
        if f.degree(var_idx) < g.degree(var_idx) {
            std::mem::swap(&mut f, &mut g);
        }
        if g.degree(var_idx) == 0 {
            return g;
        }
    ...
    pub fn partial_derivative(&self, var_idx: usize) -> Self {
        ...
        for (exps, coeff) in &self.terms {
            if var_idx < exps.len() && exps[var_idx] > 0 {
                ...
                new_exps[var_idx] -= 1;
                *result.terms.entry(new_exps).or_insert(BigInt::from(0)) +=
                    coeff * BigInt::from(exp);
            }
        }
        result.cleanup();
        result
...
impl NlaSolver {
    ...
    pub fn expr_to_poly(&mut self, expr: &Expr) -> Option<MultivariatePolynomial> {
        match expr {
            ...
            Expr::Add(args) => {
                ...
                for arg in args {
                    res = res.add(&self.expr_to_poly(arg)?);
                }
                Some(res)
            }
            Expr::Mul(args) => {
                ...
                for arg in args {
                    res = res.mul(&self.expr_to_poly(arg)?);
                }
                Some(res)
            }
            Expr::Sub(args) => {
                if args.is_empty() {
                    return Some(MultivariatePolynomial::new());
                }
                let mut res = self.expr_to_poly(&args[0])?;
                for arg in &args[1..] {
                    let mut neg = self.expr_to_poly(arg)?;
                    for v in neg.terms.values_mut() {
                        *v *= -1;
                    }
                    res = res.add(&neg);
                }
    ...
    pub fn project(
        ...
        for (i, p) in polys.iter().enumerate() {
            ...
            for q in &polys[i + 1..] {
                proj.push(p.resultant(q, var_idx));
            }
        }
...
impl TheorySolver for NlaSolver {
    ...
    fn check(&mut self) -> bool {
        ...
        for expr in self.constraints.clone() {
            match expr {
                Expr::Lt(a, b) => {
                    if let Some(p) = self.expr_to_poly(&Expr::Sub(vec![*a, *b])) {
                        polys_with_op.push((p, "lt"));
                    }
                }
                Expr::Gt(a, b) => {
                    if let Some(p) = self.expr_to_poly(&Expr::Sub(vec![*a, *b])) {
                        polys_with_op.push((p, "gt"));
                    }
                }
                Expr::Le(a, b) => {
                    if let Some(p) = self.expr_to_poly(&Expr::Sub(vec![*a, *b])) {
                        polys_with_op.push((p, "le"));
                    }
                }
                Expr::Ge(a, b) => {
                    if let Some(p) = self.expr_to_poly(&Expr::Sub(vec![*a, *b])) {
                        polys_with_op.push((p, "ge"));
                    }
                }
                Expr::Eq(a, b) => {
                    if let Some(p) = self.expr_to_poly(&Expr::Sub(vec![*a, *b])) {
                        polys_with_op.push((p, "eq"));
                    }
                }
                _ => {}
            }
        ...
        for (p, op) in polys_with_op {
            ...
            for (exps, coeff) in &p.terms {
                ...
                if coeff < &BigInt::from(0) {
                    all_coeffs_non_neg = false;
                }
                if exps.iter().all(|&e| e == 0) && coeff > &BigInt::from(0) {
                    has_const_pos = true;
                }
            }
