...
fn test_array_basic_select_store() {
    ...
    // a[i <- v][i] = v
    let a = Expr::Var("a".to_string(), Type::Array(Box::new(Type::Int), Box::new(Type::Int)));
    ...
    solver.assert(&Expr::Not(Box::new(Expr::Eq(Box::new(select_store), Box::new(v)))));
...
fn test_array_diff_indices() {
    ...
    solver.assert(&Expr::Not(Box::new(Expr::Eq(Box::new(i.clone()), Box::new(j.clone())))));
    ...
    solver.assert(&Expr::Not(Box::new(Expr::Eq(Box::new(select_store), Box::new(select_a)))));
